...
SELECT
	...
	LL.MA_MUC_AN_PHAT AS THA_AN_PHAT,
	...
	CASE
		WHEN REGEXP_LIKE(TRIM(TO_CHAR(LL.CHI_MUC_TIM_KIEM)), '^[-]?[0-9]+$')
		THEN TO_NUMBER(TRIM(TO_CHAR(LL.CHI_MUC_TIM_KIEM)))
		ELSE NULL
	END AS THA_CHI_MUC_TIEM_KIEM,
...
LEFT JOIN QLPN.DM_CAP_TOA_XUS CAP_TOA_XU ON TO_CHAR(LL.MA_CQ_TOA_XU) = TO_CHAR(CAP_TOA_XU.CTX_MA)
