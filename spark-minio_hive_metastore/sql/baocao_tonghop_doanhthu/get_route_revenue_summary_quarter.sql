...
SELECT 
'{{year_utc_7}}-{{quarter_utc_7}}' as quarter_id,
split(A.CHECKOUT_TOLL_NAME, '\\s*-\\s*')[1] AS route,
A.VEHICLE_TYPE AS vehicle_transaction_type,  -- loại phương tiện
CASE 
...
INNER JOIN ice.gold.dim_date D on A.year = D.year AND A.month = D.month
WHERE (
...
GROUP BY A.VEHICLE_TYPE, C.CYCLE_NAME,split(A.CHECKOUT_TOLL_NAME, '\\s*-\\s*')[1]
UNION ALL 
SELECT 
'{{year_utc_7}}-{{quarter_utc_7}}' as quarter_id,
split(A.CHECKOUT_TOLL_NAME, '\\s*-\\s*')[1] AS route,
...
'Đầu tư công' as cycle_type
FROM ice.gold.fact_transport_transaction_stage A
-- INNER JOIN ice.gold.dim_date D on A.year = D.year AND A.month = D.month // thừa chỗ này 
WHERE (
    {% for day_range in day_ranges %}
    (A.year = '{{ day_range.year }}'
     AND A.month = '{{ day_range.month }}'
...
GROUP BY A.checkout_channel, C.CYCLE_NAME,split(A.CHECKOUT_TOLL_NAME, '\\s*-\\s*')[1]